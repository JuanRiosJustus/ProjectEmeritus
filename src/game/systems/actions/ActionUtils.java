...
import game.systems.CombatSystem;
...
public class ActionUtils {
    ...
    private static final Logger logger = LoggerFactory.instance().logger(ActionUtils.class);

    public static void moveToTileWithinMovementRange(EngineController engine, Entity unit, Entity tile) {
        moveToTileWithinMovementRange(engine, unit, tile, null);
    }

    public static void moveToTileWithinMovementRange(EngineController engine, Entity unitToMove, Entity tileToMoveTo, Logger logger) {
        ...
        if (tileToMoveTo != manager.tileOccupying) {
            ...
            movement.move(engine, unitToMove, tileToMoveTo);
    ...
    public static void gatherTilesWithinMovementRange(EngineController engine, Entity unit, int range, Entity tile) {
        ...
        getTilesInLineOfSight(engine.model.game.model, manager.tileToMoveTo, range, manager.tilesWithinAbilityRange);

        // populate the path tiles
        TilePathing.getTilesWithinPath(engine.model.game.model, manager.tileOccupying, manager.tileToMoveTo,
                stats.getScalarNode(Constants.DISTANCE).getTotal(), manager.tilesWithinMovementRangePath);
    }

    public static void gatherTilesWithinAbilityRange(EngineController engine, Entity unit, Ability ability, Entity tile) {
        ActionManager manager = unit.get(ActionManager.class);
        ...
        getTilesInLineOfSight(engine.model.game.model, manager.tileOccupying, ability.range, manager.tilesWithinAbilityRange);
        ...
        if (ability.areaOfEffect == 0 && manager.tilesWithinAbilityRange.contains(tile)) {
            ...
            getTilesInLineOfSight(engine.model.game.model, tile, ability.areaOfEffect, manager.tilesWithinAreaOfEffect);
        }
    }

    public static void attackTileWithinAbilityRange(EngineController engine, Entity unit, Ability ability, Entity tile) {
    ...
    public static void randomlyAttack(EngineController engine, Entity unit) {
        ...
        for (Ability ability : abilities) {
            ...
            getTilesInLineOfSight(engine.model.game.model, manager.tileOccupying, ability.range, tilesWithinAbilityLOS);
            ...
            if (tilesWithEntities.isEmpty()) {
                ...
                for (Entity tileToTarget : tilesWithinAbilityLOS) {
                    ...
                    getTilesInLineOfSight(engine.model.game.model, tileToTarget, ability.areaOfEffect, tilesWithinAoeLOS);
            ...
            attackTileWithinAbilityRange(engine, unit,ability, selectedTileWithEntity);
            break;
        }
        manager.attacked = true;
    }

    public static void moveTowardsEntityIfPossible(EngineController engine, Entity unit) {
        ...
        TilePathing.getCardinalTiles(engine.model.game.model, manager.tileOccupying, cardinalTiles);
        ...
        TilePathing.getTilesInLineOfSight(engine.model.game.model, manager.tileOccupying, viewRange, tilesWithinView);
        ...
        if (tilesWithTargets.isEmpty()) {
            ...
            randomlyMove(engine, unit);
        } else {

            // Get tiles closest to the entity found
            Set<Entity> unobstructedTilesThatCanBeMovedTo = new HashSet<>();
            TilePathing.getUnobstructedTilePath(engine.model.game.model, manager.tileOccupying, viewRange, unobstructedTilesThatCanBeMovedTo);
            ...
            for (Entity tileWithTarget : tilesWithTargets) {
                ...
                TilePathing.getCardinalTiles(engine.model.game.model, tileWithTarget, cardinalTiles);
            ...
            gatherTilesWithinMovementRange(engine, unit,
                    stats.getScalarNode(Constants.DISTANCE).getTotal(), tileToMoveTo);
            moveToTileWithinMovementRange(engine, unit, tileToMoveTo);
        }
        manager.moved = true;
    }
    ...
    public static void randomlyMove(EngineController engine, Entity unit) {
        Movement movement = unit.get(Movement.class);
        ...
        if (randomTile != manager.tileOccupying) {
            // regather tiles
            gatherTilesWithinMovementRange(engine, unit, stats.getScalarNode(Constants.DISTANCE).getTotal(), randomTile);
            moveToTileWithinMovementRange(engine, unit, randomTile);
        }
        manager.moved = true;
    }

    public static Ability tryGetRangeFromLongestRangeAbility(EngineController engine, Entity unit) {
...
//    public static Ability tryGetRangeFromSelected(EngineController engine, Entity unit) {
//        Statistics stats = unit.get(Statistics.class);
