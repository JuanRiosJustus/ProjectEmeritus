...
public class CombatSystem extends GameSystem {
    ...
    public void handleCombatEndEvent(JSONObject event) {
        ...
        request.put("actor_unit_id", actorEntityID);
        request.put("ability", ability);
        request.put("acted_on_unit_id", actedOnEntityID);

        mGameModel.applyAbility(request);
    }
    ...
    public static Map<String, Float> getCostMapping(String userUnitID, String ability) {
        ...
        for (int i = 0; i < costs.size(); i++) {
            ...
            String targetAttribute = AbilityTable.getInstance().getAttribute(cost);
