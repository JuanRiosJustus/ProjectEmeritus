...
public class GameControllerV1 extends EngineScene {

    private final GameAPI mGameAPI;
    private final GameModel mGameModel;
    private final GameViewV1 mGameViewV1;
...
//            mInstance = new GameController(screenWidth, screenHeight);
    ...
    public GameControllerV1 create(int viewWidth, int viewHeight, int rows, int columns, int spriteWidth, int spriteHeight) {
...
//                .setSpriteHeight(spriteHeight);
        GameGenerationConfigs configs = GameGenerationConfigs.getDefaults()
                .setRows(rows)
                .setColumns(columns);
        GameControllerV1 newGameControllerV1 = new GameControllerV1(configs);
        return newGameControllerV1;
    }

    public static GameControllerV1 create(GameGenerationConfigs configs) {
        GameControllerV1 newGameControllerV1 = new GameControllerV1(configs);
        return newGameControllerV1;
    }

    private GameControllerV1(JSONObject configsJson, JSONArray gameMap) {
        GameGenerationConfigs configs = (GameGenerationConfigs) configsJson;
        mGameModel = new GameModel(configs, gameMap);
        mGameViewV1 = new GameViewV1(mGameModel);
        mGameAPI = new GameAPI();
    }

    private GameControllerV1(JSONObject configsJson) {
        GameGenerationConfigs configs = (GameGenerationConfigs) configsJson;
    ...
    public void update() {
        if (!mGameViewV1.isGamePanelShowing() || !mGameModel.isRunning()) { return; }
        mGameModel.update();
        mGameViewV1.update(mGameModel);
    }
    public void input() {
...
//        mGameModel.input(InputControllerV1.getInstance());
    }
    ...
    public JPanel getGamePanel(int width, int height) {
        JPanel newGamePanel = mGameViewV1.getViewPort(width, height);
        InputControllerV1.getInstance().setup(newGamePanel);
        return newGamePanel;
    }
