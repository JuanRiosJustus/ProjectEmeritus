...
public class Statistic extends JSONObject {
    public static final String FLAT = "additive";
    public static final String PERCENT = "multiplicative";
    private static final String BASE_KEY = "base";
    private static final String MISSING_KEY = "missing";
    private static final String BONUS_KEY = "modification";
    ...
    private final Queue<String> mEphemeral = new LinkedList<>();
...
//    private float mBase = 0f;
    ...
    private static final String FLAT_MAP = "flat_map";
    private static final String PERCENT_MAP = "percent_map";

    private JSONObject mFlatMap = new JSONObject();
    private JSONObject mPercentMap = new JSONObject();

    public Statistic(String name) { this(name, 0); }
    public Statistic(String name, float base) {
//        mName = name;
//        mBase = base;
//        mCurrent = mBase;
        ...
        mFlatMap = new JSONObject();
        mPercentMap = new JSONObject();
        put(FLAT_MAP, mFlatMap);
        put(PERCENT_MAP, mPercentMap);


        mDirty = true;
    }

    public void addFlatBonus(String source, String name, float value) { addBonus(source, name, FLAT, value); }
    public void addPercentBonus(String source, String name, float value) { addBonus(source, name, PERCENT, value); }
    ...
    public void fill() {
        put(CURRENT, getTotal());
    }



    public void addBonus(String source, String name, String type, float value) {

        switch (type) {
            case FLAT -> {
                mAdditiveMap.put(source, value);
                mFlatMap.put(source, value);
            }
            case PERCENT -> {
                mMultiplicativeMap.put(source, value);
                mPercentMap.put(source, value);
            }
        }
        mDirty = true;
        float t2 = getAndOrCalculateTotal();
//
...
//        mChecksum.hasChanged(base, modification, current, additiveState, multiplicativeState);
    ...
    public float getScaling(String type) {
        ...
        switch (type) {
            ...
            case TOTAL, MAX_KEY -> result = getBase() + getBonus();
...
//    }
    ...
    public int getCurrent() { return (int) getIntValue(CURRENT); }
    public int getBase() { return (int) getIntValue(BASE); }
    public int getBonus() { return getTotal() - getBase(); }
    public int getTotal() {  return (int) getAndOrCalculateTotal(); }
    public void setBase(float value) { put(BASE, value); mDirty = true; }


    public void setCurrent(double value) {
        mCurrentIsInUse = true;
        int clampedValue = (int) Math.max(0, Math.min(value, getTotal())); // Clamp between 0 and total
        put(CURRENT, clampedValue);
        mDirty = true;
    }
    ...
    private float getAndOrCalculateTotal() {
        if (!mDirty) { return getFloatValue(TOTAL);  }

        float current = getCurrent();
        float base = getBase();

        float flatBonus = 0;
        for (String source : mFlatMap.keySet()) {
            float value = mFlatMap.getFloatValue(source);
            flatBonus += value;
        }

        float percentBonus = 0.0f;
        for (String source : mPercentMap.keySet()) {
            float value = mPercentMap.getFloatValue(source);
            percentBonus += value;
        }


        float result = (base + flatBonus) * (1.0f + percentBonus);
        ...
        mHashCode = (int) (mMultiplicativeMap.hashCode() + mAdditiveMap.hashCode() + base + getCurrent() + result);
