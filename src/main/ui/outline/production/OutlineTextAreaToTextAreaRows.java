...
public class OutlineTextAreaToTextAreaRows extends GameUI {
    protected JPanel mRowsPanel = new GameUI();
    protected final Map<String, OutlineTextAreaToTextAreaRow> mRowsMap = new LinkedHashMap<>();
    protected Color mColor = null;
    protected int mVisibleRows = 0;
    protected static final int DEFAULT_VISIBLE_ROWS = 4;

    public OutlineTextAreaToTextAreaRows(int width, int height, Color color) {
        this(width, height, color, DEFAULT_VISIBLE_ROWS);
    }

    public OutlineTextAreaToTextAreaRows(int width, int height, Color color, int visibleRows) {
        super(width, height);
    ...
    public OutlineTextAreaToTextAreaRow createRow(String id) {
        ...
        OutlineTextAreaToTextAreaRow row = mRowsMap.get(id);

        if (row != null) { return row; }


        row = new OutlineTextAreaToTextAreaRow(id, "", mColor, outlineButtonPanelWidth, outlineButtonPanelHeight);
...
//        SwingUiUtils.setHoverEffect(row);
    ...
    public OutlineTextAreaToTextAreaRow getRow(String value) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            OutlineTextAreaToTextAreaRows rows = new OutlineTextAreaToTextAreaRows(400, 300, Color.YELLOW, 4);
