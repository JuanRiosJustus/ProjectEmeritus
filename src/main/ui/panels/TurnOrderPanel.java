...
public class TurnOrderPanel extends JScene {
    ...
    private final int paenlWidth = 150;
    private final int portraitWidths;
    private final int portraitHeights;
    ...
    private final Color turnIsNow = ColorPalette.TRANSPARENT_GREY;

    public TurnOrderPanel(int width, int height) {
        super(width, height, TurnOrderPanel.class.getSimpleName());

        portraitWidths = 150;//(int) (width * .3);
        portraitHeights = 90;//(int) (height * 1);
    ...
    private JPanel contentPane(int width, int height) {
        ...
        int imagesToShow = width / portraitWidths;

        for (int i = 0; i < imagesToShow; i++) {
            JButton timelineItem = new JButton();
            timelineItem.setBackground(ColorPalette.TRANSPARENT_BLACK);
            ...
            timelineItem.setOpaque(true);
            // timelineItem.setBorder(BorderFactory.createBevelBorder(0));
            queueViewPanel.add(timelineItem);
            // JImage jImage = new JImage(new ImageIcon());
            // jImage.setBorder(new EtchedBorder(ColorPalette.WHITE, ColorPalette.BEIGE));
            // jImage.setPreferredSize(new Dimension(portraitWidths, portraitHeights));
    ...
    public void update(GameModel model) {
        ...
        while (!toPlace.isEmpty()) {
            ...
            JButton image = (JButton) queueViewPanel.getComponent(indexToPlace);
            image.setIcon(icon);
        ...
        JButton image = (JButton) queueViewPanel.getComponent(0);
