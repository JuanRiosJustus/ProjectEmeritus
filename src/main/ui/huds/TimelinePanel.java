...
public class TimelinePanel extends JScene {
...
//        setBackground(ColorPalette.BLUE);
    ...
    public void jSceneUpdate(GameModel model) {
        ...
        Entity userSelected = model.getGameState().getSelectedEntity();
        boolean isNonNullAndAlreadySelecting = userSelected != null && userSelected.get(Tile.class).getUnit() == selected;
        if (first == model.mSpeedQueue.peek() && isNonNullAndAlreadySelecting) { return; }
        if (model.mSpeedQueue.peek() == null) { return; }
        first = model.mSpeedQueue.peek();
        selected = userSelected == null ? null : userSelected.get(Tile.class).getUnit();
        ...
        Color divider = ColorPalette.TRANSPARENT_GREY;
        Color selected = ColorPalette.YELLOW;
        ...
        for (TimeLineItem tli : mTimeLineItems) {
            ...
            if (entity != null) {
                ...
                imageContainer.setToolTipText(entity.get(Identity.class).getName());

                ActionListener al = e -> {
                    MovementManager movementManager = entity.get(MovementManager.class);
                    model.mGameState.put(GameState.CURRENTLY_SELECTED, movementManager.getCurrentTile());
                    model.mGameState.put(GameState.GLIDE_TO_SELECTED, true);
                };
                ...
                if (iteration == 0) {
                    imageContainer.setBackground(selected);
                    label.setBackground(selected);
                } else {
                    if (turnDividerHits > 0) {
                        imageContainer.setBackground(upcoming);
                        label.setBackground(upcoming);
                    } else {
                        imageContainer.setBackground(current);
                        label.setBackground(current);
                    }
                }
                label.setText(entity.get(Identity.class).getName());
            } else  {
                String turnText = "TURN " + (turnCounts + turnDividerHits);

                imageContainer.setBackground(divider);
                imageContainer.setText("←");
                imageContainer.setToolTipText(turnText);
                imageContainer.setIcon(null);

                label.setText(turnText);
                label.setBackground(divider);

                turnDividerHits = turnDividerHits + 1;
            }

            label.setMultiClickThreshhold(1000);
            imageContainer.setMultiClickThreshhold(1000);
