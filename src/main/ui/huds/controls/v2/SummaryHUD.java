...
//import main.game.stats.ResourceNodeModification;
...
//        String temp = mCurrentUnit.get(Identity.class).toString() + " (" + summary.getSpecies() + ")";
//        if (!nameField.getValue().equalsIgnoreCase(temp) || !forceUpdate) {
//            nameField.setValue(temp);
//        }
//
//        temp = mCurrentUnit.get(Types.class).getTypes().toString();
...
//        int buttonHeight = 0;
//
//        Tags tags = mCurrentUnit.get(Tags.class);
...
//        }
//
//        if (modCount != summary.getModificationCount() || lastViewedUnit != mCurrentUnit || !forceUpdate) {
//            modificationPanel.removeAll();
//            for (String key : summary.getStatNodeNames()) {
//                if (key.equalsIgnoreCase(Summary.EXPERIENCE)) { continue; }
//                if (key.equalsIgnoreCase(Summary.LEVEL)) { continue; }
//                StatisticsNode node = summary.getStatsNode(key);
...
//                }
//
//                for (Map.Entry<Object, ResourceNodeModification> entry : node.getModifications().entrySet()) {
