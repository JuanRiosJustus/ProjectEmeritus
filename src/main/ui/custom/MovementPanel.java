...
//            setupType = 3;
            addOrUpdateRow(Statistics.MOVE, String.valueOf(statistics.getStatTotal(Statistics.MOVE)));
            addOrUpdateRow(Statistics.SPEED, String.valueOf(statistics.getStatTotal(Statistics.SPEED)));
            addOrUpdateRow(Statistics.CLIMB, String.valueOf(statistics.getStatTotal(Statistics.CLIMB)));
            addOrUpdateRow(ELEVATION, "" + tile.getHeight());
            addOrUpdateRow(TILE, tile.toString());
        }
//
//        if (animation == null) { return; }
...
//        setup(entity, setupType);
    ...
    public void setLabelIfDifferent(String key, String value) {
        if (!DatasheetPanel.getJLabelComponent(mDatasheetPanel, key).getText().equals(value)) {
            DatasheetPanel.getJLabelComponent(mDatasheetPanel, key).setText(value);
        }
    }
    private void setup(Entity entity, int type) {
        if (true) { return; }
        if (type == 3) {
            ...
            if (!DatasheetPanel.getJLabelLabelComponent(mDatasheetPanel, Constants.LEVEL).getText().equals(lvlTxt)) {
                DatasheetPanel.getJLabelLabelComponent(mDatasheetPanel, Constants.LEVEL).setText(lvlTxt);
            }

            addOrUpdateRow(Statistics.MOVE, String.valueOf(statistics.getStatTotal(Statistics.MOVE)));
            addOrUpdateRow(Statistics.SPEED, String.valueOf(statistics.getStatTotal(Statistics.SPEED)));
            addOrUpdateRow(Statistics.CLIMB, String.valueOf(statistics.getStatTotal(Statistics.CLIMB)));
            addOrUpdateRow(ELEVATION, "5");
            addOrUpdateRow(TILE, String.valueOf(statistics.getStatTotal(Statistics.CLIMB)));
//            setLabelIfDifferent(Constants.NAME, entity.toString());
